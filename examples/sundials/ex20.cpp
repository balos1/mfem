...
//
// Sample runs:  ex20
//               ex20 -p 1 -s 2 -o 1 -n 120 -dt 0.1
//               ex20 -p 1 -s 2 -o 2 -n 60 -dt 0.2
//               ex20 -p 1 -s 2 -o 3 -n 40 -dt 0.3
//               ex20 -p 1 -s 2 -o 4 -n 30 -dt 0.4
//
...
public:
  ...
  void ExplicitMult(const Vector &x, Vector &y) const override;
};
...
int main(int argc, char *argv[])
{
  ...
  if (ode_solver_type == 1) {
    siaSolver = std::make_unique<SIAVSolver>(order);
    siaSolver->Init(P,F);
  } else if (ode_solver_type == 2) {
    auto sprk_solver = new SPRKStepSolver();
    sprk_solver->Init(P,F);
    sprk_solver->SetOrder(order);
...
void NegGradV::ExplicitMult(const Vector &x, Vector &y) const
{
  Mult(x, y);
}
